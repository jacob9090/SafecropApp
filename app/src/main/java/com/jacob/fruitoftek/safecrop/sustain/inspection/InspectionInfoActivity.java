...
public class InspectionInfoActivity extends AppCompatActivity {

    private PreferenceHelper preferenceHelper;
    private ProgressBar syncInspectionInfoProgressBar;
    private TextView syncInspectionInfoCountTextView;
    private SharedPreferences sharedPreferences;

    private Button syncInspectionInfoStartDateButton, syncInspectionInfoEndDateButton, syncInspectionInfoFilterButton;
    private String syncInspectionInfoStartDate = "", syncInspectionInfoEndDate = "";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Spinner chartFilterSpinner = findViewById(R.id.syncInspectionInfoChartFilterSpinner);
        ...
        syncInspectionInfoProgressBar = findViewById(R.id.syncInspectionInfoProgressBar);
        syncInspectionInfoCountTextView = findViewById(R.id.syncInspectionInfoCountTV);
        sharedPreferences = getSharedPreferences("SyncCache", MODE_PRIVATE);

        syncInspectionInfoStartDateButton = findViewById(R.id.syncInspectionInfoStartDateBtn);
        syncInspectionInfoEndDateButton = findViewById(R.id.syncInspectionInfoEndDateBtn);
        syncInspectionInfoFilterButton = findViewById(R.id.syncInspectionInfoFilterBtn);

        syncInspectionInfoStartDateButton.setOnClickListener(view -> showDatePicker(true));
        syncInspectionInfoEndDateButton.setOnClickListener(view -> showDatePicker(false));

        syncInspectionInfoFilterButton.setOnClickListener(view -> {
            if (!syncInspectionInfoStartDate.isEmpty() && !syncInspectionInfoEndDate.isEmpty()) {
                fetchSyncCount(userEmail, syncInspectionInfoStartDate, syncInspectionInfoEndDate);
            } else {
        ...
        if (userEmail == null || userEmail.trim().isEmpty()) {
            ...
            syncInspectionInfoCountTextView.setText("Error: Missing Name Data");
        } else {
    ...
    private void fetchSyncCount(String userEmail, String startDate, String endDate) {
        runOnUiThread(() -> syncInspectionInfoProgressBar.setVisibility(View.VISIBLE));
        new Thread(() -> {
            try {
                String urlString = "https://app.safecropgh.org/sus_cert/sync_inspection_info.php?userEmail=" +
                ...
                if (responseCode != HttpURLConnection.HTTP_OK) {
                    ...
                    runOnUiThread(() -> {
                        ...
                        syncInspectionInfoCountTextView.setText("Error fetching sync count");
                    });
                    return;
                }
                ...
                try {
                    ...
                    if ("success".equals(status)) {
                        runOnUiThread(() -> {
                            syncInspectionInfoProgressBar.setVisibility(View.GONE);
                            syncInspectionInfoCountTextView.setText("Total Records Synced: " + totalRecords);
                        });
                    } else {
                        runOnUiThread(() -> {
                            syncInspectionInfoProgressBar.setVisibility(View.GONE);
                            syncInspectionInfoCountTextView.setText("No records found");
                        });
                    }
    ...
    private void showDatePicker(boolean isStartDate) {
        ...
        DatePickerDialog datePickerDialog = new DatePickerDialog(
                ...
                (view, year, month, dayOfMonth) -> {
                    ...
                    if (isStartDate) {
                        syncInspectionInfoStartDate = selectedDate;
                        syncInspectionInfoStartDateButton.setText("Start: " + selectedDate);
                        Log.d("SyncSurveyInfo", "Selected Start Date: " + syncInspectionInfoStartDate);
                    } else {
                        syncInspectionInfoEndDate = selectedDate;
                        syncInspectionInfoEndDateButton.setText("End: " + selectedDate);
                        Log.d("SyncSurveyInfo", "Selected End Date: " + syncInspectionInfoEndDate);
                    }
                },
    ...
    private void fetchSurveyDataForChart(String userEmail, String filterType) {
        new Thread(() -> {
            try {
                String urlString = "https://app.safecropgh.org/sus_cert/sync_inspection_chart.php?userEmail=" +
                        URLEncoder.encode(userEmail, "UTF-8") +
    ...
    private void loadCachedSyncCount() {
        ...
        syncInspectionInfoCountTextView.setText("Total Records Synced (Cached): " + cachedCount);
        syncInspectionInfoProgressBar.setVisibility(View.GONE);
    }
    ...
    private void updateBarChart(List<BarEntry> entries, List<String> labels) {
        BarChart barChart = findViewById(R.id.syncInspectionInfoBarChart);
