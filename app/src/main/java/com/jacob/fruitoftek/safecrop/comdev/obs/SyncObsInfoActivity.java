...
public class SyncObsInfoActivity extends AppCompatActivity {
    
    private PreferenceHelper preferenceHelper;
    private ProgressBar syncObsInfoProgressBar;
    private TextView syncObsInfoCountTextView;
    private SharedPreferences sharedPreferences;

    private Button syncObsInfoStartDateButton, syncObsInfoEndDateButton, syncObsInfoFilterButton;
    private String syncObsInfoStartDate = "", syncObsInfoEndDate = "";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        EdgeToEdge.enable(this);
        setContentView(R.layout.activity_sync_obs_info);
        ...
        Spinner chartFilterSpinner = findViewById(R.id.syncObsInfoChartFilterSpinner);
        ...
        syncObsInfoProgressBar = findViewById(R.id.syncObsInfoProgressBar);
        syncObsInfoCountTextView = findViewById(R.id.syncObsInfoCountTV);
        sharedPreferences = getSharedPreferences("SyncCache", MODE_PRIVATE);

        syncObsInfoStartDateButton = findViewById(R.id.syncObsInfoStartDateBtn);
        syncObsInfoEndDateButton = findViewById(R.id.syncObsInfoEndDateBtn);
        syncObsInfoFilterButton = findViewById(R.id.syncObsInfoFilterBtn);

        syncObsInfoStartDateButton.setOnClickListener(view -> showDatePicker(true));
        syncObsInfoEndDateButton.setOnClickListener(view -> showDatePicker(false));

        syncObsInfoFilterButton.setOnClickListener(view -> {
            if (!syncObsInfoStartDate.isEmpty() && !syncObsInfoEndDate.isEmpty()) {
                fetchSyncCount(userEmail, syncObsInfoStartDate, syncObsInfoEndDate);
            } else {
        ...
        if (userEmail == null || userEmail.trim().isEmpty()) {
            ...
            syncObsInfoCountTextView.setText("Error: Missing Name Data");
        } else {
    ...
    private void fetchSyncCount(String userEmail, String startDate, String endDate) {
        runOnUiThread(() -> syncObsInfoProgressBar.setVisibility(View.VISIBLE));
        new Thread(() -> {
            try {
                String urlString = "https://fruitoftek.com/fedco/clmrs/sync_obs_info.php?userEmail=" +
                ...
                if (responseCode != HttpURLConnection.HTTP_OK) {
                    ...
                    runOnUiThread(() -> {
                        syncObsInfoProgressBar.setVisibility(View.GONE);
                        syncObsInfoCountTextView.setText("Error fetching sync count");
                    });
                    return;
                }
                ...
                try {
                    ...
                    if ("success".equals(status)) {
                        runOnUiThread(() -> {
                            syncObsInfoProgressBar.setVisibility(View.GONE);
                            syncObsInfoCountTextView.setText("Total Records Synced: " + totalRecords);
                        });
                    } else {
                        runOnUiThread(() -> {
                            syncObsInfoProgressBar.setVisibility(View.GONE);
                            syncObsInfoCountTextView.setText("No records found");
                        });
                    }
                    ...
                    runOnUiThread(() -> {
                        syncObsInfoProgressBar.setVisibility(View.GONE);
                        syncObsInfoCountTextView.setText("Error parsing sync count");
                    });
                }
    ...
    private void showDatePicker(boolean isStartDate) {
        ...
        DatePickerDialog datePickerDialog = new DatePickerDialog(
                ...
                (view, year, month, dayOfMonth) -> {
                    ...
                    if (isStartDate) {
                        syncObsInfoStartDate = selectedDate;
                        syncObsInfoStartDateButton.setText("Start: " + selectedDate);
                        Log.d("SyncSurveyInfo", "Selected Start Date: " + syncObsInfoStartDate);
                    } else {
                        syncObsInfoEndDate = selectedDate;
                        syncObsInfoEndDateButton.setText("End: " + selectedDate);
                        Log.d("SyncSurveyInfo", "Selected End Date: " + syncObsInfoEndDate);
                    }
                },
    ...
    private void fetchSurveyDataForChart(String userEmail, String filterType) {
        new Thread(() -> {
            try {
                String urlString = "https://fruitoftek.com/fedco/clmrs/sync_obs_chart.php?userEmail=" +
                        URLEncoder.encode(userEmail, "UTF-8") +
    ...
    private void loadCachedSyncCount() {
        ...
        syncObsInfoCountTextView.setText("Total Records Synced (Cached): " + cachedCount);
        syncObsInfoProgressBar.setVisibility(View.GONE);
    }
    ...
    private void updateBarChart(List<BarEntry> entries, List<String> labels) {
        BarChart barChart = findViewById(R.id.syncObsInfoBarChart);
