...
public class TrainingInfoActivity extends AppCompatActivity {
    ...
    private ProgressBar syncTrainingInfoProgressBar;
    private TextView syncTrainingInfoCountTextView;
    private SharedPreferences sharedPreferences;

    private Button syncTrainingInfoStartDateButton, syncTrainingInfoEndDateButton, syncTrainingInfoFilterButton;
    private String syncTrainingInfoStartDate = "", syncTrainingInfoEndDate = "";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Spinner chartFilterSpinner = findViewById(R.id.syncTrainingInfoChartFilterSpinner);
        ...
        syncTrainingInfoProgressBar = findViewById(R.id.syncTrainingInfoProgressBar);
        syncTrainingInfoCountTextView = findViewById(R.id.syncTrainingInfoCountTV);
        sharedPreferences = getSharedPreferences("SyncCache", MODE_PRIVATE);

        syncTrainingInfoStartDateButton = findViewById(R.id.syncTrainingInfoStartDateBtn);
        syncTrainingInfoEndDateButton = findViewById(R.id.syncTrainingInfoEndDateBtn);
        syncTrainingInfoFilterButton = findViewById(R.id.syncTrainingInfoFilterBtn);

        syncTrainingInfoStartDateButton.setOnClickListener(view -> showDatePicker(true));
        syncTrainingInfoEndDateButton.setOnClickListener(view -> showDatePicker(false));

        syncTrainingInfoFilterButton.setOnClickListener(view -> {
            if (!syncTrainingInfoStartDate.isEmpty() && !syncTrainingInfoEndDate.isEmpty()) {
                fetchSyncCount(userEmail, syncTrainingInfoStartDate, syncTrainingInfoEndDate);
            } else {
        ...
        if (userEmail == null || userEmail.trim().isEmpty()) {
            ...
            syncTrainingInfoCountTextView.setText("Error: Missing Name Data");
        } else {
    ...
    private void fetchSyncCount(String userEmail, String startDate, String endDate) {
        runOnUiThread(() -> syncTrainingInfoProgressBar.setVisibility(View.VISIBLE));
        new Thread(() -> {
            try {
                String urlString = "https://app.safecropgh.org/sus_cert/sync_training_info.php?userEmail=" +
                ...
                if (responseCode != HttpURLConnection.HTTP_OK) {
                    ...
                    runOnUiThread(() -> {
                        syncTrainingInfoProgressBar.setVisibility(View.GONE);
                        syncTrainingInfoCountTextView.setText("Error fetching sync count");
                    });
                    return;
                }
                ...
                try {
                    ...
                    if ("success".equals(status)) {
                        runOnUiThread(() -> {
                            syncTrainingInfoProgressBar.setVisibility(View.GONE);
                            syncTrainingInfoCountTextView.setText("Total Records Synced: " + totalRecords);
                        });
                    } else {
                        runOnUiThread(() -> {
                            syncTrainingInfoProgressBar.setVisibility(View.GONE);
                            syncTrainingInfoCountTextView.setText("No records found");
                        });
                    }
                    ...
                    runOnUiThread(() -> {
                        syncTrainingInfoProgressBar.setVisibility(View.GONE);
                        syncTrainingInfoCountTextView.setText("Error parsing sync count");
                    });
                }
    ...
    private void showDatePicker(boolean isStartDate) {
        ...
        DatePickerDialog datePickerDialog = new DatePickerDialog(
                ...
                (view, year, month, dayOfMonth) -> {
                    ...
                    if (isStartDate) {
                        syncTrainingInfoStartDate = selectedDate;
                        syncTrainingInfoStartDateButton.setText("Start: " + selectedDate);
                        Log.d("SyncSurveyInfo", "Selected Start Date: " + syncTrainingInfoStartDate);
                    } else {
                        syncTrainingInfoEndDate = selectedDate;
                        syncTrainingInfoEndDateButton.setText("End: " + selectedDate);
                        Log.d("SyncSurveyInfo", "Selected End Date: " + syncTrainingInfoEndDate);
                    }
                },
    ...
    private void fetchSurveyDataForChart(String userEmail, String filterType) {
        new Thread(() -> {
            try {
                String urlString = "https://app.safecropgh.org/sus_cert/sync_training_chart.php?userEmail=" +
                        URLEncoder.encode(userEmail, "UTF-8") +
    ...
    private void loadCachedSyncCount() {
        ...
        syncTrainingInfoCountTextView.setText("Total Records Synced (Cached): " + cachedCount);
        syncTrainingInfoProgressBar.setVisibility(View.GONE);
    }
    ...
    private void updateBarChart(List<BarEntry> entries, List<String> labels) {
        BarChart barChart = findViewById(R.id.syncTrainingInfoBarChart);
