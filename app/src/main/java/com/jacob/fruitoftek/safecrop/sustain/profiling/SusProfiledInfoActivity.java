...
public class SusProfiledInfoActivity extends AppCompatActivity {
    
    private PreferenceHelper preferenceHelper;
    private ProgressBar syncSusProfilingInfoProgressBar;
    private TextView syncSusProfilingInfoCountTextView;
    private SharedPreferences sharedPreferences;

    private Button syncSusProfilingInfoStartDateButton, syncSusProfilingInfoEndDateButton, syncSusProfilingInfoFilterButton;
    private String syncSusProfilingInfoStartDate = "", syncSusProfilingInfoEndDate = "";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        EdgeToEdge.enable(this);
        setContentView(R.layout.activity_sus_profiled_info);
        ...
        Spinner chartFilterSpinner = findViewById(R.id.syncSusProfilingInfoChartFilterSpinner);
        ...
        syncSusProfilingInfoProgressBar = findViewById(R.id.syncSusProfilingInfoProgressBar);
        syncSusProfilingInfoCountTextView = findViewById(R.id.syncSusProfilingInfoCountTV);
        sharedPreferences = getSharedPreferences("SyncCache", MODE_PRIVATE);

        syncSusProfilingInfoStartDateButton = findViewById(R.id.syncSusProfilingInfoStartDateBtn);
        syncSusProfilingInfoEndDateButton = findViewById(R.id.syncSusProfilingInfoEndDateBtn);
        syncSusProfilingInfoFilterButton = findViewById(R.id.syncSusProfilingInfoFilterBtn);

        syncSusProfilingInfoStartDateButton.setOnClickListener(view -> showDatePicker(true));
        syncSusProfilingInfoEndDateButton.setOnClickListener(view -> showDatePicker(false));

        syncSusProfilingInfoFilterButton.setOnClickListener(view -> {
            if (!syncSusProfilingInfoStartDate.isEmpty() && !syncSusProfilingInfoEndDate.isEmpty()) {
                fetchSyncCount(userEmail, syncSusProfilingInfoStartDate, syncSusProfilingInfoEndDate);
            } else {
        ...
        if (userEmail == null || userEmail.trim().isEmpty()) {
            ...
            syncSusProfilingInfoCountTextView.setText("Error: Missing Name Data");
        } else {
    ...
    private void fetchSyncCount(String userEmail, String startDate, String endDate) {
        runOnUiThread(() -> syncSusProfilingInfoProgressBar.setVisibility(View.VISIBLE));
        new Thread(() -> {
            try {
                String urlString = "https://app.safecropgh.org/sus_cert/sync_profiling_info.php?userEmail=" +
                ...
                if (responseCode != HttpURLConnection.HTTP_OK) {
                    ...
                    runOnUiThread(() -> {
                        syncSusProfilingInfoProgressBar.setVisibility(View.GONE);
                        syncSusProfilingInfoCountTextView.setText("Error fetching sync count");
                    });
                    return;
                }
                ...
                try {
                    ...
                    if ("success".equals(status)) {
                        runOnUiThread(() -> {
                            syncSusProfilingInfoProgressBar.setVisibility(View.GONE);
                            syncSusProfilingInfoCountTextView.setText("Total Records Synced: " + totalRecords);
                        });
                    } else {
                        runOnUiThread(() -> {
                            syncSusProfilingInfoProgressBar.setVisibility(View.GONE);
                            syncSusProfilingInfoCountTextView.setText("No records found");
                        });
                    }
                    ...
                    runOnUiThread(() -> {
                        syncSusProfilingInfoProgressBar.setVisibility(View.GONE);
                        syncSusProfilingInfoCountTextView.setText("Error parsing sync count");
                    });
                }
    ...
    private void showDatePicker(boolean isStartDate) {
        ...
        DatePickerDialog datePickerDialog = new DatePickerDialog(
                ...
                (view, year, month, dayOfMonth) -> {
                    ...
                    if (isStartDate) {
                        syncSusProfilingInfoStartDate = selectedDate;
                        syncSusProfilingInfoStartDateButton.setText("Start: " + selectedDate);
                        Log.d("SyncSurveyInfo", "Selected Start Date: " + syncSusProfilingInfoStartDate);
                    } else {
                        syncSusProfilingInfoEndDate = selectedDate;
                        syncSusProfilingInfoEndDateButton.setText("End: " + selectedDate);
                        Log.d("SyncSurveyInfo", "Selected End Date: " + syncSusProfilingInfoEndDate);
                    }
                },
    ...
    private void fetchSurveyDataForChart(String userEmail, String filterType) {
        new Thread(() -> {
            try {
                String urlString = "https://app.safecropgh.org/sus_cert/sync_profiling_chart.php?userEmail=" +
                        URLEncoder.encode(userEmail, "UTF-8") +
    ...
    private void loadCachedSyncCount() {
        ...
        syncSusProfilingInfoCountTextView.setText("Total Records Synced (Cached): " + cachedCount);
        syncSusProfilingInfoProgressBar.setVisibility(View.GONE);
    }
    ...
    private void updateBarChart(List<BarEntry> entries, List<String> labels) {
        BarChart barChart = findViewById(R.id.syncSusProfilingInfoBarChart);
