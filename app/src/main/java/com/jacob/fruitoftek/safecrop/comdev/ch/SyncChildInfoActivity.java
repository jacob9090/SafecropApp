...
public class SyncChildInfoActivity extends AppCompatActivity {
    
    private PreferenceHelper preferenceHelper;
    private ProgressBar syncChInfoProgressBar;
    private TextView syncChInfoCountTextView;
    private SharedPreferences sharedPreferences;

    private Button syncChInfoStartDateButton, syncChInfoEndDateButton, syncChInfoFilterButton;
    private String syncChInfoStartDate = "", syncChInfoEndDate = "";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        EdgeToEdge.enable(this);
        setContentView(R.layout.activity_sync_child_info);
        ...
        Spinner chartFilterSpinner = findViewById(R.id.syncChInfoChartFilterSpinner);
        ...
        syncChInfoProgressBar = findViewById(R.id.syncChInfoProgressBar);
        syncChInfoCountTextView = findViewById(R.id.syncChInfoCountTV);
        sharedPreferences = getSharedPreferences("SyncCache", MODE_PRIVATE);

        syncChInfoStartDateButton = findViewById(R.id.syncChInfoStartDateBtn);
        syncChInfoEndDateButton = findViewById(R.id.syncChInfoEndDateBtn);
        syncChInfoFilterButton = findViewById(R.id.syncChInfoFilterBtn);

        syncChInfoStartDateButton.setOnClickListener(view -> showDatePicker(true));
        syncChInfoEndDateButton.setOnClickListener(view -> showDatePicker(false));

        syncChInfoFilterButton.setOnClickListener(view -> {
            if (!syncChInfoStartDate.isEmpty() && !syncChInfoEndDate.isEmpty()) {
                fetchSyncCount(userEmail, syncChInfoStartDate, syncChInfoEndDate);
            } else {
        ...
        if (userEmail == null || userEmail.trim().isEmpty()) {
            ...
            syncChInfoCountTextView.setText("Error: Missing Name Data");
        } else {
    ...
    private void fetchSyncCount(String userEmail, String startDate, String endDate) {
        runOnUiThread(() -> syncChInfoProgressBar.setVisibility(View.VISIBLE));
        new Thread(() -> {
            try {
                String urlString = "https://fruitoftek.com/fedco/clmrs/sync_child_info.php?userEmail=" +
                ...
                if (responseCode != HttpURLConnection.HTTP_OK) {
                    ...
                    runOnUiThread(() -> {
                        syncChInfoProgressBar.setVisibility(View.GONE);
                        syncChInfoCountTextView.setText("Error fetching sync count");
                    });
                    return;
                }
                ...
                try {
                    ...
                    if ("success".equals(status)) {
                        runOnUiThread(() -> {
                            syncChInfoProgressBar.setVisibility(View.GONE);
                            syncChInfoCountTextView.setText("Total Records Synced: " + totalRecords);
                        });
                    } else {
                        runOnUiThread(() -> {
                            syncChInfoProgressBar.setVisibility(View.GONE);
                            syncChInfoCountTextView.setText("No records found");
                        });
                    }
                    ...
                    runOnUiThread(() -> {
                        syncChInfoProgressBar.setVisibility(View.GONE);
                        syncChInfoCountTextView.setText("Error parsing sync count");
                    });
                }
    ...
    private void showDatePicker(boolean isStartDate) {
        ...
        DatePickerDialog datePickerDialog = new DatePickerDialog(
                ...
                (view, year, month, dayOfMonth) -> {
                    ...
                    if (isStartDate) {
                        syncChInfoStartDate = selectedDate;
                        syncChInfoStartDateButton.setText("Start: " + selectedDate);
                        Log.d("SyncSurveyInfo", "Selected Start Date: " + syncChInfoStartDate);
                    } else {
                        syncChInfoEndDate = selectedDate;
                        syncChInfoEndDateButton.setText("End: " + selectedDate);
                        Log.d("SyncSurveyInfo", "Selected End Date: " + syncChInfoEndDate);
                    }
                },
    ...
    private void fetchSurveyDataForChart(String userEmail, String filterType) {
        new Thread(() -> {
            try {
                String urlString = "https://fruitoftek.com/fedco/clmrs/sync_child_chart.php?userEmail=" +
                        URLEncoder.encode(userEmail, "UTF-8") +
    ...
    private void loadCachedSyncCount() {
        ...
        syncChInfoCountTextView.setText("Total Records Synced (Cached): " + cachedCount);
        syncChInfoProgressBar.setVisibility(View.GONE);
    }
    ...
    private void updateBarChart(List<BarEntry> entries, List<String> labels) {
        BarChart barChart = findViewById(R.id.syncChInfoBarChart);
