...
public class ComDevLocalBottomSheet extends BottomSheetDialogFragment {

    private ComDbHelper comDbHelper;
    private SchoolDbHelper schoolDbHelper;
    private HhDbHelper hhDbHelper;
    private ObsDbHelper obsDbHelper;
    private ChildSurveyDBHelper dbHelper;

    @Override
    public View onCreateView(LayoutInflater inflater, @Nullable
    ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.bottom_sheet_comdev_local, container, false);

        comDbHelper = new ComDbHelper(getActivity());
        schoolDbHelper = new SchoolDbHelper(getActivity());
        ...
        TextView cdbsLocalBackupComTv = view.findViewById(R.id.cdbsLocalBackupComTv);
        cdbsLocalBackupComTv.setOnClickListener(v -> exportComToCSV());

        TextView cdbsCloudBackupSchoolTv = view.findViewById(R.id.cdbsCloudBackupSchoolTv);
        cdbsCloudBackupSchoolTv.setOnClickListener(v -> exportSchoolToCSV());

        TextView cdbsCloudBackupHouseholdTv = view.findViewById(R.id.cdbsCloudBackupHouseholdTv);
        cdbsCloudBackupHouseholdTv.setOnClickListener(v -> exportHouseholdToCSV());

        TextView cdbsCloudBackupObservationTv = view.findViewById(R.id.cdbsCloudBackupObservationTv);
        cdbsCloudBackupObservationTv.setOnClickListener(v -> exportObsToCSV());

        TextView cdbsCloudBackupChTv = view.findViewById(R.id.cdbsCloudBackupChTv);
        cdbsCloudBackupChTv.setOnClickListener(v -> exportChildToCSV());

        return view;
    }

    private void exportComToCSV() {
        new Thread(() -> {
            ...
            File csvFile = new File(csvBackupDir, "CommunitySurveyBackup.csv");
            ...
            try (FileWriter writer = new FileWriter(csvFile)) {
                ...
                for (ComModel com : comList) {
                    writer.append(escapeCSV(com.getCommquestion1())).append(',')
                            ...
                            .append(escapeCSV(com.getFarmer_photo() != null ? com.getFarmer_photo().toString() : "")).append(',')
                ...
                getActivity().runOnUiThread(() -> {
                    Toast.makeText(getActivity(), "CSV Exported Successfully", Toast.LENGTH_SHORT).show();
                });
    ...
    private void exportSchoolToCSV() {
        new Thread(() -> {
            ...
            File csvFile = new File(csvBackupDir, "SchoolSurveyBackup.csv");
            ...
            try (FileWriter writer = new FileWriter(csvFile)) {
                ...
                for (SchoolModel school : schoolList) {
                    writer.append(escapeCSV(school.getSch_name())).append(',')
                            ...
                            .append(escapeCSV(school.getFarmer_photo() != null ? school.getFarmer_photo().toString() : "")).append(',')
    ...
    private void exportHouseholdToCSV() {
        new Thread(() -> {
            ...
            File csvFile = new File(csvBackupDir, "HouseholdSurveyBackup.csv");
    ...
    private void exportObsToCSV() {
        new Thread(() -> {
            ...
            File csvFile = new File(csvBackupDir, "ObservationSurveyBackup.csv");
