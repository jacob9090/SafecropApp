...
public class SyncComInfoActivity extends AppCompatActivity {
    
    private PreferenceHelper preferenceHelper;
    private ProgressBar syncComInfoProgressBar;
    private TextView syncComInfoCountTextView;
    private SharedPreferences sharedPreferences;

    private Button syncComInfoStartDateButton, syncComInfoEndDateButton, syncComInfoFilterButton;
    private String syncComInfoStartDate = "", syncComInfoEndDate = "";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        EdgeToEdge.enable(this);
        setContentView(R.layout.activity_sync_com_info);
        ...
        Spinner chartFilterSpinner = findViewById(R.id.syncComInfoChartFilterSpinner);
        ...
        syncComInfoProgressBar = findViewById(R.id.syncComInfoProgressBar);
        syncComInfoCountTextView = findViewById(R.id.syncComInfoCountTV);
        sharedPreferences = getSharedPreferences("SyncCache", MODE_PRIVATE);

        syncComInfoStartDateButton = findViewById(R.id.syncComInfoStartDateBtn);
        syncComInfoEndDateButton = findViewById(R.id.syncComInfoEndDateBtn);
        syncComInfoFilterButton = findViewById(R.id.syncComInfoFilterBtn);

        syncComInfoStartDateButton.setOnClickListener(view -> showDatePicker(true));
        syncComInfoEndDateButton.setOnClickListener(view -> showDatePicker(false));

        syncComInfoFilterButton.setOnClickListener(view -> {
            if (!syncComInfoStartDate.isEmpty() && !syncComInfoEndDate.isEmpty()) {
                fetchSyncCount(userEmail, syncComInfoStartDate, syncComInfoEndDate);
            } else {
        ...
        if (userEmail == null || userEmail.trim().isEmpty()) {
            ...
            syncComInfoCountTextView.setText("Error: Missing Name Data");
        } else {
    ...
    private void fetchSyncCount(String userEmail, String startDate, String endDate) {
        runOnUiThread(() -> syncComInfoProgressBar.setVisibility(View.VISIBLE));
        new Thread(() -> {
            try {
                String urlString = "https://fruitoftek.com/fedco/clmrs/sync_com_info.php?userEmail=" +
                ...
                if (responseCode != HttpURLConnection.HTTP_OK) {
                    ...
                    runOnUiThread(() -> {
                        syncComInfoProgressBar.setVisibility(View.GONE);
                        syncComInfoCountTextView.setText("Error fetching sync count");
                    });
                    return;
                }
                ...
                try {
                    ...
                    if ("success".equals(status)) {
                        runOnUiThread(() -> {
                            syncComInfoProgressBar.setVisibility(View.GONE);
                            syncComInfoCountTextView.setText("Total Records Synced: " + totalRecords);
                        });
                    } else {
                        runOnUiThread(() -> {
                            syncComInfoProgressBar.setVisibility(View.GONE);
                            syncComInfoCountTextView.setText("No records found");
                        });
                    }
                    ...
                    runOnUiThread(() -> {
                        syncComInfoProgressBar.setVisibility(View.GONE);
                        syncComInfoCountTextView.setText("Error parsing sync count");
                    });
                }
    ...
    private void showDatePicker(boolean isStartDate) {
        ...
        DatePickerDialog datePickerDialog = new DatePickerDialog(
                ...
                (view, year, month, dayOfMonth) -> {
                    ...
                    if (isStartDate) {
                        syncComInfoStartDate = selectedDate;
                        syncComInfoStartDateButton.setText("Start: " + selectedDate);
                        Log.d("SyncSurveyInfo", "Selected Start Date: " + syncComInfoStartDate);
                    } else {
                        syncComInfoEndDate = selectedDate;
                        syncComInfoEndDateButton.setText("End: " + selectedDate);
                        Log.d("SyncSurveyInfo", "Selected End Date: " + syncComInfoEndDate);
                    }
                },
    ...
    private void fetchSurveyDataForChart(String userEmail, String filterType) {
        new Thread(() -> {
            try {
                String urlString = "https://fruitoftek.com/fedco/clmrs/sync_com_chart.php?userEmail=" +
                        URLEncoder.encode(userEmail, "UTF-8") +
    ...
    private void loadCachedSyncCount() {
        ...
        syncComInfoCountTextView.setText("Total Records Synced (Cached): " + cachedCount);
        syncComInfoProgressBar.setVisibility(View.GONE);
    }
    ...
    private void updateBarChart(List<BarEntry> entries, List<String> labels) {
        BarChart barChart = findViewById(R.id.syncComInfoBarChart);
